...
class RealtimeclimPipeline(object):
    def __init__(self):
    ...
    def process_item(self, item, spider):
        ...
        area_create = "create table if not exists `areas_data`\
                    ...
                    `station_name`            varchar(60)        null default null comment '资料来源',\
                    primary key (area_id)\
        ...
        if areadict[item['area']] in self.currents_seen:
            cursor.execute(area_update, (item['area'], areadict[item['area']]))
        else:
            cursor.execute(area_insert, (item['area'], areadict[item['area']]))
        ...
        data_insert = "insert into aqi_cli_data(`datetime`, `qx_Humidity`, `qx_Pressure`, `qx_Temp`, `area_id`) values(%s,%s,%s,%s,%s)"
        try:
            cursor.execute(data_insert,(item['datetime'], item['qx_humidity'], item['qx_pressure'], item['qx_temp'], str(result[0])))
            dbObject.commit()
        except Exception as e:
            print(e)
            dbObject.rollback()
